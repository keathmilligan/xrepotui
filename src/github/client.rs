...
impl GitHubClient {
    ...
    pub async fn fetch_repo_summary(
        ...
        self.cache_repo_summary.insert(
            key,
            CacheEntry {
                value: value.clone(),
    ...
    async fn do_fetch_repo_summary(
        &self,
        owner: &str,
        repo: &str,
    ) -> Result<RepoSummary, ClientError> {
    ...
    pub async fn fetch_recent_commits(
        ...
        let value = self
            .do_fetch_recent_commits(owner, repo, branch, limit)
            .await?;
        self.cache_commits.insert(
            key,
            CacheEntry {
                value: value.clone(),
        ...
        let changed_files = files
            ...
            .map(|f| {
                ...
                let change_type = match f.status {
                    ...
                    DiffEntryStatus::Modified | DiffEntryStatus::Changed | _ => {
    ...
    pub async fn fetch_workflow_runs(
        ...
        self.cache_workflow_runs.insert(
            key,
            CacheEntry {
                value: value.clone(),
    ...
    async fn do_fetch_workflow_runs(
        &self,
        owner: &str,
        repo: &str,
        limit: u8,
    ...
    pub async fn fetch_workflow_run_jobs(
        ...
        let key = CacheKey::WorkflowRunJobs {
            owner: owner.to_string(),
            repo: repo.to_string(),
            run_id,
        };
    ...
    async fn do_fetch_workflow_run_jobs(
        &self,
        owner: &str,
        repo: &str,
        run_id: u64,
        ...
        let bytes = self
            .with_retry(|| {
                ...
                async move {
                    octocrab
                        .actions()
                        .download_workflow_run_logs(&o, &r, RunId(run_id))
                        .await
                }
            })
            .await?;
        ...
        for attempt in 0..max {
            match f().await {
                ...
                Err(e) => {
                    ...
                    if last_err.contains("rate limit")
                        || last_err.contains("403")
